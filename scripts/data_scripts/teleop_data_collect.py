...
class Calibrate:
    ...
    def __init__(self, directory, fileName):
        ...
        self._gripperSensors = np.zeros(7)
        self._gripperPosition = 0
        ...
        rospy.Subscriber('/gripper_sensors', Float32MultiArray, self._callbackGripperSensors)
    ...
    def _callbackGripperSensors(self, dataGripperSensors):
        self._gripperSensors[0] = dataGripperSensors.data[0]
        self._gripperSensors[1] = dataGripperSensors.data[1]
        self._gripperSensors[2] = dataGripperSensors.data[2]
        self._gripperSensors[3] = dataGripperSensors.data[3]
        self._gripperSensors[4] = dataGripperSensors.data[4]
        self._gripperSensors[5] = dataGripperSensors.data[5]
        self._gripperSensors[6] = np.sum(dataGripperSensors.data)
    ...
    def _callbackBaseFeedback(self, dataFeedback):
        self._gripperPosition = dataFeedback.interconnect.oneof_tool_feedback.gripper_feedback[0].motor[0].position
    ...
    def recordData(self):
        ...
        currentTime = (time.time() * 1000) - self._startTime
        ...
        gripperData = str(self._gripperSensors[0]) + ',' + \
                      str(self._gripperSensors[1]) + ',' + \
                      str(self._gripperSensors[2]) + ',' + \
                      str(self._gripperSensors[3]) + ',' + \
                      str(self._gripperSensors[4]) + ',' + \
                      str(self._gripperSensors[5]) + ',' + \
                      str(self._gripperSensors[6])
        
        self._fileCSV.write(str(currentTime) + ',' + str(mocapData) + ',' + 
            str(robotData) + ',' + str(jointData) + ',' + str(gripperData) + '\n')
...
def main():
    ...
    self._fileCSV.write(str(endTime - startTime))
