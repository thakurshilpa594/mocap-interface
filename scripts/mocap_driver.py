...
class MocapDriverNode:
    ...
    def __init__(self):
        ...
        # link lengths
        self.L1 = 0.20 # m
        # Sternum to shoulder = 0.20 m
        self.L2 = 0.33 # m
        # Shoulder to elbow = 0.33 m
        self.L3 = 0.30 # m
        # Elbow to wrist = 0.30 m

        # shoulder offset
        #shoulderXOffset = -0.100
        shoulderXOffset = -0.40
        #shoulderYOffset = 0.0
        shoulderYOffset = 0.10
        #shoulderZOffset = self.L1 + self.L2/2.0
        shoulderZOffset = 0.24

        # workspace transfer scaling factor
        # userMaxReach  = np.sqrt(np.power(self.L2, 2) + 2*self.L1*self.L2) + shoulderXOffset
        # userMaxReach =  np.sqrt(np.power(self.L1,2) + np.power((self.L2 + self.L3),2))
        # print(userMaxReach)
        # robotMaxReach = 0.902 # 0.750 m by hit-and-trial and 0.902 m from Kinova Gen3 7DOF Robot datasheet.
        # self.WORKSPACESCALINGFACTOR = robotMaxReach / userMaxReach
        self.WORKSPACESCALINGFACTOR = 1
        print("Workspace scaling factor: " + str(self.WORKSPACESCALINGFACTOR))
        ...
        # self._af = r.from_euler('xyz', (225, 0, 0), degrees=True)
        # self._af = r.from_euler('xyz', (270, 0, 0), degrees=True)
        self._af = r.from_euler('xyz', (180, 0, 0), degrees=True)

        # need a rotation to align mocap ee with robot ee
        self._ef = r.from_euler('xyz', (0, -90, 0), degrees=True) # 180 to match ee
        ...
        self._pubMocapPos_Simple = rospy.Publisher('/mocap_ee_pose_simple',          Point,  queue_size=10)

        #Make this into a vector of 3 things (POINT MSG)
    ...
    def publishDataToROS(self):
        ...
        modelEE.pose.position.x    = self.position[0]*self.WORKSPACESCALINGFACTOR
        modelEE.pose.position.y    = self.position[1]*self.WORKSPACESCALINGFACTOR
        modelEE.pose.position.z    = self.position[2]*self.WORKSPACESCALINGFACTOR
        ...
        mocapPoint = Point()
        mocapPoint.x = modelEE.pose.position.x - 0.25
        mocapPoint.y = modelEE.pose.position.y + 0.25
        mocapPoint.z = modelEE.pose.position.z

        self._pubMocapPos.publish(mocapPose)
        self._pubMocapPos_Simple.publish(mocapPoint)
